...
Names are one of the single most important things in a program.
```

Wait, that's the Zen of Python. Here's ours:

---

```
No parentheses for function calls.
Add is cleaner than add().
...
Kō has no parentheses for function calls. No curly braces. No semicolons. The grammar fits on a page. The spec fits on a few pages. If you can't explain it simply, you don't understand it well enough.
...
- Runs everywhere a C compiler runs
- Easy to debug (read the generated C)
...
2. **No curly braces.** Indentation defines blocks.
3. **No semicolons.** Newlines separate expressions.
...
10. **Compile to C.** Run everywhere.
