...
- **`println` of a generic function parameter prints the wrong thing.** `println`'s output format is chosen from a type tag baked in at compile time; a value that reaches `println` through a genuinely polymorphic parameter (e.g. `fn show n = println n` called as both `show 42` and `show "hello"`) never gets a concrete tag, so it falls back to printing as a raw integer — coincidentally correct for `Int`, wrong for `Bool` (`1`/`0` instead of `True`/`False`), and garbage (a pointer address) for `String` and records. Workaround: only `println` values that are literals or bindings built in the same function, not values forwarded through a generic parameter. Fixing this properly needs monomorphization or runtime type tags on all values, not a local patch.
